...
namespace NUnitTestProjectDAL
{
    public class Tests
    {
        ...
        public void Setup()
        {
            ...
            _lstCustomer.Add(customer);

            _Findcustomer = new Customer();
            _Findcustomer.Id = 2;
            _Findcustomer.LN = "van den Berg";

            _lstCustomer.Add(_Findcustomer);
        }
        ...
        public void GetAll_Should_Return_List_Customers()
        {
            ...
            dalMock.Setup(x => x.GetAll()).Returns(_lstCustomer);
            ...
            //assert
            Assert.AreEqual(2, _lstCustomer.Count);
        }
        ...
        public void FindByID_Should_Return_a_Customer()
        {
            //arrange
            var dalMock = new Mock<IDAL>();
            dalMock.Setup(mr => mr.FindById(It.IsAny<int>())).Returns((int s) => _lstCustomer.Where(x => x.Id == s).Single());
        ...
        public void FindByID_With_InvalidID_Should_Return_a_NULL()
        {
            //arrange
            var dalMock = new Mock<IDAL>();
            dalMock.Setup(mr => mr.FindById(It.IsAny<int>())).Returns((int s) => _lstCustomer.Find(x => x.Id == s));
